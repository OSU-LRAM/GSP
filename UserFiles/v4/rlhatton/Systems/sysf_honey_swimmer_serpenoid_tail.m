function output = sysf_honey_swimmer_serpenoid_tail(input_mode)
	...
	switch input_mode
		...
		case 'name'
			...
			output = 'Low Re Serpenoid first modes tail'; % Display name

		case 'dependency'

			output.dependency = {'Utilities/curvature_mode_toolbox/backbone_from_curvature_bases.m',
				'Utilities/curvature_mode_toolbox/curvatures/serpenoid_1_tail.m',
				'Utilities/curvature_mode_toolbox/curvatures/serpenoid_2_tail.m',
		...
		case 'initialize'
			...
			s.density.scalar = [5 5]; %density to display scalar functions
			s.density.eval = [5 5];   %density for function evaluations
			s.finite_element_density = 5;
			% power metric
			s.metric = @(x,y) LowRE_dissipation_metric_from_curvature_bases...
				({@serpenoid_1_tail;@serpenoid_2_tail},[x;y],1,1);
...
function A = A_num_helper(a1,a2)
	...
	A = LowRE_local_connection_from_curvature_bases({@serpenoid_1_tail;@serpenoid_2_tail},[a1;a2],1,1);
