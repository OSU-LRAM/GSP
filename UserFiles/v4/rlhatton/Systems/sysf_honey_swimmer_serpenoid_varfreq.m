function output = sysf_honey_swimmer_serpenoid_varfreq(input_mode)
	...
	switch input_mode
		...
		case 'name'
			...
			output = 'Low Re Serpenoid first modes variable frequency'; % Display name

		case 'dependency'

			output.dependency = {'Utilities/curvature_mode_toolbox/backbone_from_general_curvature.m';
				'Utilities/curvature_mode_toolbox/curvatures/curv_serpenoid_varfreq';
		...
		case 'initialize'
			...
			s.density.eval = [1,1,1]*11;%[21 21 21];   %density for function evaluations
			s.finite_element_density = 11;%11;
			% power metric
			s.metric = @(x,y,z) LowRE_dissipation_metric_from_general_curvature...
				(@curv_serpenoid_varfreq,[x;y;z],1,1,2);  % @(x,y) eye(2);%
...
% 	addpath(genpath('/Users/rlhatton/Documents/MATLAB/LowRE_toolbox/'))
	...
	A = LowRE_local_connection_from_general_curvature(@curv_serpenoid_varfreq,[a1;a2;a3],1,1,2);
