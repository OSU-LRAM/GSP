function output = sysf_mudskipper(input_mode)
...
% file, instead of generating it programmatically.
    ...
    Local_connection = 'constrained_high_adduction_20';
	...
	switch input_mode
		...
		case 'name'
			...
			output = 'Mud Skipper'; % Display name
		...
		case 'initialize'
            ...
            load(Local_connection,'A','alpha1','alpha2')
            %load(Metric)

			%Functional representation of local connection
			s.A_num = @(a1,a2) Conn_num(a1,a2,alpha1',alpha2',A{1}',A{4}',A{2}',A{5}',A{3}',A{6}');
			s.A_den = @Conn_den;
