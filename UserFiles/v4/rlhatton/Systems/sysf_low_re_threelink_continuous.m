function output = sysf_low_re_threelink_continuous(input_mode)
	...
	switch input_mode
		...
		case 'name'
			...
			output = 'Viscous 3-link swimmer'; % Display name
		...
		case 'initialize'
			...
			s.finite_element_density = 11; % density of finite-element solution to optimized coordinates
			% power metric
			s.metric = @(x,y) LowRE_dissipation_metric_from_curvature_bases...
				({@serpenoid_1;@serpenoid_2},[x;y],1,1,4);
			%%%
			%Display parameters
...
% 	addpath(genpath('/Users/rlhatton/Documents/MATLAB/LowRE_toolbox/'))
	...
	A = LowRE_local_connection_from_curvature_bases({@discrete_1;@discrete_2},[a1;a2],1,1,4);
