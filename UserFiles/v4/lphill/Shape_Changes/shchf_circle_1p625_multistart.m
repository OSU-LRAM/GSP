function output = shchf_circle_1p625_multistart(input_mode)
 %[r1,r2] = convert.old_to_new_points(alpha1,alpha2);
	...
	switch input_mode
		...
		case 'name'
			...
			output = 'circle 1.625 multistart';
		...
		case 'initialize'
			...
			%path definition
			A=1.625; % Amplitdude of circle
			P = linspace(pi/2, 3*pi/2,15); % Starting phase
